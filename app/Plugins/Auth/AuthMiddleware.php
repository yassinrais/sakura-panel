...
class AuthMiddleware extends ControllerBase implements MiddlewareInterface , SharedConstInterface
{
    ...
    private $auth_data = ['id','username','email' , 'role_name', 'role', 'status'];
    ...
    public function beforeExecuteRoute(Dispatcher $dispatcher)
    {
        ...
        $role_name = $user->role_name  ?? $this::ROLE_DEFAULT;
        ...
        if (!$this->acl->isAllowed($role_name, $controllerName, $actionName)) {
            if (!$this->request->isAjax()){
                ...
                /** 
                 * Check Resource:Access / Create One if its not existed ! 
                 */
                $resource = Resources::findOrCreate(
                    ["name = :name:" , "bind"=>  ["name" => $controllerName] ],
                    ["description" => ucfirst($controllerName)]
                );
                $access = Accesses::findOrCreate(
                    ["resource_id = :resource_id: ", "bind" => ["resource_id" => $resource->id] ],
                    ["name"=> $actionName ,"description"=> ucfirst($actionName)]
                );
                $permission = RolePermissions::findOrCreate(
                    ["role_id = :role_id: AND resource_id = :resource_id: AND access_id = :access_id:",
                        "bind"=>[
                    ...
                    [
                        "allowed" => $this::INACTIVE
                    ]
                );

                /** 
                 * Return Dispatcher forward page error 
                 */
